...
function Astar(grid,ROW,COL){
    ...
    while(pq.length!==0){
        ...
        priority-=calculateHValue(x,y,end);
        if(valid(x+1,y,ROW,COL)&&!visited[x+1][y]){
          pq.queue({element:[x+1,y],priority:newGrid[x+1][y].weight+priority+calculateHValue(x+1,y,end)});
          visited[x+1][y]=true;
        ...
        if(valid(x,y+1,ROW,COL)&&!visited[x][y+1]){
          pq.queue({element:[x,y+1],priority:newGrid[x][y+1].weight+priority+calculateHValue(x,y+1,end)});
          visited[x][y+1]=true;
